...
bool psEngine::Initialize(int level)
{
    ...
    if(level == 0)
    {
      ...
      //  csReport(object_reg, CS_REPORTER_SEVERITY_NOTIFY, PSAPP, APPNAME);
        csReport(object_reg, CS_REPORTER_SEVERITY_NOTIFY, CS_VERSION,
                 "This game uses the Crystal Space Engine created by Jorrit and others");
      //  csReport(object_reg, CS_REPORTER_SEVERITY_NOTIFY, PSAPP, CS_VERSION);
        ...
        //Check if sound is on or off in euclient.cfg
...
#endif
    psCSSetup* CSSetup = new psCSSetup(argc, argv, "/this/euclient.cfg", CONFIGFILENAME);
