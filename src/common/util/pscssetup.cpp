...
{ 
    ...
    {
        csReport(object_reg, CS_REPORTER_SEVERITY_ERROR, "euclient",
    ...
    if (!pluginRequest)
    {
        csReport(object_reg, CS_REPORTER_SEVERITY_ERROR, 
               "euclient", "Failed to initialize plugins!");
        return 0;
    }
    ...
    {
        csReport (object_reg, CS_REPORTER_SEVERITY_ERROR,
            "euclient", "No 3d driver (iGraphics3D) plugin!");
        PS_PAUSEEXIT(1);
    }
...
void psCSSetup::MountUserData()
{
    ...
    // - On Linux, use ~/.PlanShift instead of ~/.crystalspace/PlanShift.
    csString configPath = csGetPlatformConfigPath("Eulora");
    configPath.ReplaceAll("/.crystalspace/", "/.");

    // Alternatively, you can set it in ps client.cfg.
