...
bool psEffectObjSpire::Load(iDocumentNode *node, iLoaderContext* ldr_context)
{
    ...
    {
        csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Attempting to create an effect obj without a material.\n");
        return false;
    }
...
bool psEffectObjSpire::PostSetup()
{
    ...
    if (!mat)
    {
        csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Couldn't find effect material: %s\n", materialName.GetData());
        return false;
    }
