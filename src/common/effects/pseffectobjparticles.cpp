...
bool psEffectObjParticles::Render(const csVector3 &up)
{
    ...
    if (!mesh)
    {
        csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Couldn't load desired mesh!\n");
        return false;
    }
    ...
    //if (!pstate)
    //{
    //    csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Effect obj is of type particles, but the mesh is not!\n");
    //    return false;
    //}
