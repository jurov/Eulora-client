...
bool psEffect::Load(iDocumentNode * node, iView * parentView, psEffect2DRenderer * renderer2d, iLoaderContext * ldr_context)
{
    ...
    if (name.IsEmpty())
    {
        csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Attempting to create an effect with no name.\n");
        return false;
    }
    ...
    while (xmlbinds->HasNext())
    {
        ...
        if (anchor)
        {
            ...
            {
                csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Problem with an effect anchor in: %s\n", name.GetData());
            }
        }
        else
        {
            csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", "Couldn't create anchor.\n");
        }
    }
    ...
    while (xmlbinds->HasNext())
    {
        ...
        if (obj)
        {
            ...
            {
                csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_ERROR, "Eulora_effects", 
    ...
    for (size_t i=0; i<effectObjs.GetSize(); ++i)
    {
        if (effectObjs[i]->Render(up))
        {
            ...
            else
            {
                ...
                {
                    csReport(psCSSetup::object_reg, CS_REPORTER_SEVERITY_WARNING, "Eulora_effects",
