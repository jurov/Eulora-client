...
//printf("TYPE - MODE- mode msg");
    ...
    msg->Add(mode);
    msg->Add(value);
//printf("ActorID: %u Mode: %u Value: %u", actorID.Unbox(), mode, value);
    // Sets valid flag based on message overrun state
    valid=!(msg->overrun);
}
...
psStatDRMessage::psStatDRMessage(MsgEntry* me)
{
    ...
    else
    {
        ...
        if(statsDirty & DIRTY_VITAL_BP)
        {
            bp = me->GetFloat();
        }

        if(statsDirty & DIRTY_VITAL_BP_RATE)
        {
            bp_rate = me->GetFloat();
        }
...
//---------------------
        if(statsDirty & DIRTY_VITAL_SPIRIT)
        {
            spirit = me->GetFloat();
        }
...
//-----------------------
        ...
        if(statsDirty & DIRTY_VITAL_SPIRIT_RATE)
        {
            spirit_rate = me->GetFloat();
        }
...
csString psStatDRMessage::ToString(NetBase::AccessPointers* /*accessPointers*/)
{
    ...
    else
    {
        ...
        if(statsDirty & DIRTY_VITAL_BP_RATE)
        {
            msgtext.AppendFmt(" BP RATE: %.2f",bp_rate);
        }
...
//---------------------
        if(statsDirty & DIRTY_VITAL_SPIRIT_RATE)
        {
            msgtext.AppendFmt(" SPIRIT RATE: %.2f",spirit_rate);
        }
...
psStatsMessage::psStatsMessage(uint32_t client, float maxHP,float maxBP, float maxMana,float maxSpirit, float maxWeight, float maxCapacity)
//psStatsMessage::psStatsMessage(uint32_t client, float maxHP, float maxMana, float maxWeight, float maxCapacity)
{
    msg.AttachNew(new MsgEntry(sizeof(float)*6));
// change float from 4 to 5 to 6
    msg->Add(maxHP);
    msg->Add(maxBP);
    msg->Add(maxMana);
    msg->Add(maxSpirit);
...
csString psStatsMessage::ToString(NetBase::AccessPointers* /*accessPointers*/)
{
    ...
    {
        msgtext.AppendFmt("HP: %.3f BP: %.3f Mana: %.3f Spirit: %.3f Weight: %.3f Capacity: %.3f",
                          hp, bp, mana, spirit, weight, capacity);
    }
...
psGUISkillMessage::psGUISkillMessage(MsgEntry* message)
{
    ...
    if(includeStats)
    {
        ...
        constitution = message->GetUInt32();
        intelligence = message->GetUInt32();
        wisdom = message->GetUInt32();
        insight = message->GetUInt32();
        agility = message->GetUInt32();
        dexterity = message->GetUInt32();
        coordination = message->GetUInt32();
        attention = message->GetUInt32();
        focus = message->GetUInt32();
        patience = message->GetUInt32();
        patiences = message->GetUInt32();
        hitpoints = message->GetUInt32();
        bloodpoints = message->GetUInt32();
        mana = message->GetUInt32();
        spirit = message->GetUInt32();
        physStamina = message->GetUInt32();
        menStamina = message->GetUInt32();

        hitpointsMax = message->GetUInt32();
        bloodpointsMax = message->GetUInt32();
        manaMax = message->GetUInt32();
        spiritMax = message->GetUInt32();
...
csString psGUISkillMessage::ToString(NetBase::AccessPointers* /*accessPointers*/)
{
    ...
    if(includeStats)
    {
        msgtext.AppendFmt("Str: %d End: %d Con: %d Int: %d Wis: %d Ins: %d Agi: %d Dex: %d Coo: %d Att: %d Foc: %d Pat: %d Pit: %d",
            strength, endurance, constitution, intelligence, wisdom, insight, agility, dexterity, coordination, attention, focus, patience,patiences);
        msgtext.AppendFmt("HP: %d (Max: %d) BP: %d (Max: %d) Mana: %d (Max: %d) Spirit: %d (Max: %d) ", hitpoints, hitpointsMax,bloodpoints, bloodpointsMax, mana, manaMax, spirit, spiritMax);
...
PSF_IMPLEMENT_MSG_FACTORY(psGUISkillsMessage, MSGTYPE_GUISKILL);

const char* psGUISkillsMessage::SkillCommandString[] =
{"REQUEST","BUY_SkILL","SKILL_LIST","SKILL_SELECTED","DESCRIPTION","QUIT"};


psGUISkillsMessage::psGUISkillsMessage(uint8_t command,
                                     csString commandData)
{
    msg.AttachNew(new MsgEntry(sizeof(bool) + sizeof(uint8_t) +
                               commandData.Length() + 1 +
...
psGUISkillsMessage::psGUISkillsMessage( uint32_t clientNum,
                                      uint8_t command,
                                      csString commandData,
                                      psSkillCache *skills,
                                      uint32_t str,
...
psGUISkillsMessage::psGUISkillsMessage(MsgEntry* message)
{
    if(!message)
        return;
...
csString psGUISkillsMessage::ToString(NetBase::AccessPointers* /*accessPointers*/)
{
    csString msgtext;
    ...
    if(includeStats)
    {
        msgtext.AppendFmt("Str: %d End: %d Con: %d Int: %d Wis: %d Ins: %d Agi: %d Dex: %d Coo: %d Att: %d Foc: %d Pat: %d ",
        ...
        int maxCoppers, float exchangeFee, bool forceOpen)
{
    ...
    msg->Add(argents);
    msg->Add(coppers);
    msg->Add(bitcentsBanked);
    msg->Add(denariusBanked);
    msg->Add(argentsBanked);
    msg->Add(coppersBanked);
    msg->Add(maxBITCents);
    msg->Add(maxDenarius);
    msg->Add(maxArgents);
    msg->Add(maxCoppers);
...
{
    msg.AttachNew(new MsgEntry(sizeof(containerID)+1 + sizeof(slotID) + sizeof(clearSlot) + strlen(itemName)+1 + strlen(icon)+1 + strlen(meshName)+1 + strlen(materialName) + 1 + sizeof(stackCount) + 4*sizeof(uint32_t) ));
    msg->SetType(MSGTYPE_UPDATE_ITEM);
//printf(" update item msg started \n");
    msg->clientnum = to;
//printf(" update item msg to \n");
    msg->Add(containerID.Unbox());
//printf(" update item msg containerid \n");
    msg->Add(slotID);
//printf(" update item msg slotid \n");
    msg->Add(clearSlot);
//printf(" update item msg clearslot \n");
    msg->Add(itemName);
//printf(" update item msg itemname \n");
    msg->Add(icon);
//printf(" update item msg icon \n");
    msg->Add(stackCount);
//printf(" update item msg stack count \n");
    msg->Add(ownerID.Unbox());
//printf(" update item msg owner id \n");
    msg->Add(msgstrings->Request(meshName).GetHash());
//printf(" update item msg meshname \n");
    msg->Add(msgstrings->Request(materialName).GetHash());
//printf(" update item msg materialname \n");
//printf(" update item msg built \n");
}
...
//printf(" view update item msg built \n");
}
                                       ...
                                       int coppers, int argents, int bitcents,int denarius)
{
    ...
    msg->Add((uint32_t)coppers);
    msg->Add((uint32_t)argents);
    msg->Add((uint32_t)bitcents);
    msg->Add((uint32_t)denarius);
}
...
psExchangeMoneyMsg::psExchangeMoneyMsg(MsgEntry* me)
{
    ...
    coppers = me->GetUInt32();
    argents = me->GetUInt32();
    bitcents = me->GetUInt32();
    denarius = me->GetUInt32();
}
...
csString psExchangeMoneyMsg::ToString(NetBase::AccessPointers* /*accessPointers*/)
{
    ...
    msgtext.AppendFmt("Container: %d %d coppers, %d argents, %d bitcents, %d denarius",
                      container, coppers, argents, bitcents, denarius);
